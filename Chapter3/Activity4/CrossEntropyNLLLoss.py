...
class Classifier(nn.Module):
    ...
    def __init__(self, input_size, ce):
        super().__init__()
...
def datapreprocessor():
    ...
    # Loading excel data
    data = pd.read_excel('defaultofcreditcardclients.xls', skiprows=1)
...
if __name__ == "__main__":
    ...
    for ce in celist:
        ...
        model = Classifier(x_train.shape[1], ce)
        ...
        ax[k].plot(range(epochs), train_losses)
        ax[k].plot(range(epochs), valid_losses)
        ax[k].set_xlabel(r'Epochs')
        ax[k].set_ylabel(r'Losses')
        ax[k].grid()
        if ce:
            ax[k].set_title('Cross Entropy Loss')
        else:
            ax[k].set_title('Negative least likelihood with log-softmax')
    ...
    fig.savefig('LossComparison.pdf', bbox_inches='tight')
