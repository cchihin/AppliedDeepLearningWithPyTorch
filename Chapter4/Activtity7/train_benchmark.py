...
if __name__ == "__main__":
    ...
    batches = [50, 100, 500, 1000]
    drop_outs = [0, 0.2, 0.4]

    fig, ax = plt.subplots(len(batches), len(drop_outs))
    ...
    for batch_size in batches:
        ...
        for drop_out in drop_outs:
            ...
            ax[i][j].plot(range(epochs), train_losses, '-o', label='Train')
            ax[i][j].plot(range(epochs), valid_losses, '-o', label='Valid')
            ax[i][j].set_xlabel('Epochs')
            ax[i][j].set_ylabel('Losses')
            ax[i][j].grid()
            ax[i][j].set_title(f'Drop out: {drop_out}, Batch size: {batch_size}')

            ax2 = ax[i][j].twinx()
            ax2.plot(range(epochs), train_accs, '--o', label='Train')
            ax2.plot(range(epochs), valid_accs, '--o', label='Valid')
            ax2.legend()
    ...
    fig.set_size_inches(len(drop_outs)*8, len(batches)*5)
