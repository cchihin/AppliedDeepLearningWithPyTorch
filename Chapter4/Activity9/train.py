...
class ConvNet(nn.Module):
    ...
    def __init__(self, dropout_prob, batch_norm):
...
if __name__ == "__main__":
    ...
    k = 0
    batch_norm_list = [1, 0]
    fig, ax = plt.subplots(len(batch_norm_list),2)
    ...
    for batch_norm in batch_norm_list:
        ...
        dropout = 0.2
        model = ConvNet(dropout, batch_norm)
        # Pushing to GPU
        ...
        ax[k][0].grid()
        if k == 0:
            ax[k][0].set_title('With Batch norm' + '\n' + f'Test Loss: {test_loss:.3f}')
        else:
            ax[k][0].set_title('Without Batch norm' + '\n' + f'Test Loss: {test_loss:.3f}')
        ...
        ax[k][1].grid()
        if k == 0:
            ax[k][1].set_title('With Batch norm' + '\n' + f'Test Acc: {test_acc:.3f}')
        else:
            ax[k][1].set_title('Without Batch norm' + '\n' + f'Test Acc: {test_acc:.3f}')

        k += 1

    # fig.set_size_inches(10*len(transform_list),6)
    fig.set_size_inches(10,6*len(batch_norm_list))
    fig.savefig(f'tf_dropout{dropout}_batch{batch_size}_compare_batchnorm.pdf', bbox_inches='tight')
